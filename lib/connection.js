...
Connection.prototype.generateBindDefinitions = function (options, bindParams) {
    if (options && !options.bindDefs && bindParams && bindParams.length) {
        var bindParamsObj = bindParams[0];
        ...
        for (index = 0; index < keys.length; index++) {
            ...
            if (typeof item === 'object') {
                if (typeof item.getDate === 'function') {
                    options.bindDefs[key] = {
                        type: constants.dateType
                    };
                } else {
                    if (typeof item.val === 'string') {
                        options.bindDefs[key].type = constants.stringType;
                        options.bindDefs[key].maxSize = constants.stringMaxSize;
                    }

                    delete options.bindDefs[key].val;
                }
