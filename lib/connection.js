 ...
 * @param {Array} [options.returningInfo] - Array of objects holding columnName and bindVarName which will be added to the RETURNING ... INTO ... clause (only used if lobMetaInfo is provided)
 ...
 * @param {object} [options.lobMetaInfo] - For LOB support this object must hold a mapping between DB column name and bind variable name
 * @param {Array} [options.returningInfo] - Array of objects holding columnName and bindVarName which will be added to the RETURNING ... INTO ... clause (only used if lobMetaInfo is provided), see connection.insert example
 ...
 * @param {Array} [additionalReturningInfo] - Array of objects holding columnName and bindVarName which will be added to the RETURNING ... INTO ... clause (only used if lobMetaInfo is provided)
...
Connection.prototype.generateReturningClause = function (sql, lobMetaInfo, lobColumns, additionalReturningInfo) {
    ...
    if (additionalReturningInfo) {
        for (index = 0; index < additionalReturningInfo.length; index++) {
            buffer.push(', ');
            buffer.push(additionalReturningInfo[index].columnName);
        }
    }
    ...
    if (additionalReturningInfo) {
        for (index = 0; index < additionalReturningInfo.length; index++) {
            buffer.push(', ');
            buffer.push(':');
            buffer.push(additionalReturningInfo[index].bindVarName);
        }
    }
