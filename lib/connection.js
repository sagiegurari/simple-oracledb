...
Connection.prototype.execute = function () {
    ...
    this.diagnosticInfo = this.diagnosticInfo || {};
        ...
        asyncLib.retry({
            ...
            asyncLib.series([
                function breakOperations(asyncReleaseCallback) {
                    if (force) {
                        self.break(function onBreak(error) {
                            if (error) {
                                debug('Unable to break connection operations, ', error.stack);
                            }
    ...
    return function onCallback(error, results) {
        if (error || (!results)) {
            ...
            case 2: //options.streamResults
                resultSetReader.stream(results.metaData, results.resultSet, options, stream);
