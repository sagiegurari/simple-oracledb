        ...
        if ((!error) && results) {
            if (results.rowsAffected >= 1 && lobColumns && lobColumns.length) {
                ...
                if (useExecuteMany) {
                    ...
                    var tasks = [];
                    var createTask = function (outBinds, lobExecuteData) {
 ...
 * @param {Object} [options.lobMetaInfo] - For LOB support this object must hold a mapping between DB column name and bind variable name
 * @param {Object} [options.returningInfo] - columnName/bindVarName pairs which will be added to the RETURNING ... INTO ... clause (only used if lobMetaInfo is provided), see connection.insert example
 * @param {Boolean} [options.useExecuteMany=false] - If true and connection.executeMany is supported, it will be used, otherwise this function will call execute per bind values row
 * @param {AsyncCallback} [callback] - Invoked with an error or the insert results (if LOBs are provided, the callback will be triggered after they have been fully written to the DB)
 * @returns {Promise} In case of no callback provided in input and promise is supported, this function will return a promise
 * @example
 * ```js
...
Connection.prototype.generateBindDefinitions = function (options, bindParams) {
    if (options && !options.bindDefs && bindParams && bindParams.length) {
        ...
        var keys = Object.keys(options.bindDefs);
        var index;
        var key;
        var item;
        for (index = 0; index < keys.length; index++) {
            key = keys[index];
            item = bindParams[0][key];
        ...
        var bindParamIndex;
        var bindParamItem;
        for (bindParamIndex = 0; bindParamIndex < bindParams.length; bindParamIndex++) {
            bindParamItem = bindParams[bindParamIndex];

            for (index = 0; index < keys.length; index++) {
                key = keys[index];
                item = bindParamItem[key];
