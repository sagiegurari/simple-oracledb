...
Pool.prototype.getConnection = function (callback) {
    ...
    var getConnectionAsync = function (promiseCallback) {
        callback = callback || promiseCallback;

        var wrapperCallback = function onConnectionCreated(error, connection) {
        ...
        }, Connection.wrapOnConnection(wrapperCallback));
    };

    return promiseHelper.run(getConnectionAsync, callback);
};
...
Pool.prototype.run = function (action, options, callback) {
    ...
    var runAsync = function (promiseCallback) {
        callback = callback || promiseCallback;
    ...
    return promiseHelper.run(runAsync, callback);
};
...
Pool.prototype.terminate = function (callback) {
    ...
    var releaseAsync = function (promiseCallback) {
        promiseCallback = promiseCallback || self.noop;
