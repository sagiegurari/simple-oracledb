...
Connection.prototype.batchInsertOrUpdate = function (insert, sql, bindParamsArray, options, callback) {
    ...
    if (options.useExecuteMany) {
        ...
        asyncLib.series(tasks, function onBatchDone(error, results) {
            const batchCallback = self.createModifyCallback(callback, commit, results);
            batchCallback(error);
        });
    }
};
...
Connection.prototype.run = function (actions, options, callback) {
    ...
    if (actions) {
        ...
        const createAsync = function (action) {
...
Connection.prototype.transaction = function (actions, options, callback) {
    ...
    if (self.inTransaction) {
        ...
        self.run(actions, options, function onTransactionEnd(error, results) {
            const transactionCallback = self.createModifyCallback(callback, true, results);
...
Connection.prototype.executeFile = function (file, options, callback) {
    ...
    const input = self.getExecuteArguments(file, [], options, callback);
 ...
 * @param {Object} [additionalReturningInfo] - columnName/bindconstName pairs which will be added to the RETURNING ... INTO ... clause (only used if lobMetaInfo is provided)
...
Connection.prototype.generateReturningClause = function (sql, lobMetaInfo, lobColumns, additionalReturningInfo) {
    ...
    for (let index = 0; index < lobColumns.length; index++) {
        if (index > 0) {
            buffer.push(', ');
        }

        const columnName = lobColumns[index];

        buffer.push(columnName);
    }

    let returnColumnNames;
    if (additionalReturningInfo) {
        returnColumnNames = Object.keys(additionalReturningInfo);

        for (let index = 0; index < returnColumnNames.length; index++) {
            buffer.push(', ');
 ...
 * In addition it will modify the bind constiables to specify the OUT bind to enable access to the LOB object.
 *
 * @function
 * @memberof! Connection
 * @private
...
Connection.prototype.modifyParams = function (sql, bindParams, options) {
    ...
    if (bindParams && options && options.lobMetaInfo) {
        ...
        //modify SQL by adding RETURNING clause and modify bind constiables definition
        if (lobColumns && lobColumns.length) {
            autoCommit = options.autoCommit;
            options.autoCommit = false; //must disable auto commit to support LOBs

            const additionalReturningInfo = options.returningInfo;
            ...
            for (let index = 0; index < lobColumns.length; index++) {
                ...
                const bindconstiableName = lobMetaInfo[columnName];
                const lobValue = bindParams[bindconstiableName];
                lobData[bindconstiableName] = lobValue;

                if (typeof lobValue === 'string') { //CLOB
                    bindParams[bindconstiableName] = {
...
/**
 * Internal function used to create the options.bindDefs used by oracledb
 * for a single column.
 *
 * @function
 * @memberof! Connection
 * @private
 * @param {Object} bindDefinitions - The options.bindDefs object
 * @param {String} key - The column name
 * @param {Object|String|Number|Date} item - The bind params item
 * @example
 * ```js
 * connection.generateBindDefinitionEntry(options.bindDefs, 'name', 'myname');
 * ```
 */
Connection.prototype.generateBindDefinitionEntry = function (bindDefinitions, key, item) {
    if (typeof item === 'object') {
        ...
        } else if (typeof item.getDate === 'function') {
            bindDefinitions[key] = {
                type: constants.dateType
            };
        } else {
            if (typeof item.val === 'string') {
                bindDefinitions[key].type = constants.stringType;
                bindDefinitions[key].maxSize = constants.stringMaxSize;
            }

            delete bindDefinitions[key].val;
        }
    } else if (typeof item === 'string') {
        bindDefinitions[key] = {
            type: constants.stringType,
            maxSize: constants.stringMaxSize
        };
        ...
        throw new Error('Unsupported bind param: ' + key);
    }
};
 ...
 * @param {Array} [bindParams] - The bind params array
 * @example
 * ```js
 * connection.generateBindDefinitions(options, bindParams);
 * ```
 */
Connection.prototype.generateBindDefinitions = function (options, bindParams) {
    if (options && !options.bindDefs && bindParams && bindParams.length) {
        //clone value
        options.bindDefs = JSON.parse(JSON.stringify(bindParams[0]));

        const keys = Object.keys(options.bindDefs);
        for (let index = 0; index < keys.length; index++) {
            const key = keys[index];
            const item = bindParams[0][key];

            this.generateBindDefinitionEntry(options.bindDefs, key, item);
        }

        for (let bindParamIndex = 0; bindParamIndex < bindParams.length; bindParamIndex++) {
            const bindParamItem = bindParams[bindParamIndex];

            for (let index = 0; index < keys.length; index++) {
                const key = keys[index];
                const item = bindParamItem[key];

                if (item != null && typeof item === 'object' && typeof item.getDate !== 'function') {
                    bindParamItem[key] = item.val;
                }
            }
        }
    }
...
Connection.prototype.handleQueryResults = function (results, callback, options, handleType, stream) {
    const metaData = this.getMetaData(results);
...
Connection.prototype.createModifyCallback = function (callback, commit, output) {
    ...
    const onWrapperCallback = function (error) {
        if (error) {
            if (commit) {
 ...
 *  const metaData = connection.getMetaData(results);
 *  console.log(metaData);
 * });
 * ```
 */
...
module.exports = {
    ...
    extend: function extend(connection) {
        if (connection && (!connection.simplified)) {
            let properties = Object.keys(Connection.prototype);
            ...
            const extendedCapabilities = extensions.get('connection');
